...
public class VerificationUtil {
    ...
    public static boolean PasswordCorrect(String password, Context context){
        ...
        if (password.equals("")){
            ...
            /*til_password.setError("");*/
            Toast.makeText(context, "密码由6-14位的英文+数字组成", Toast.LENGTH_SHORT).show();
            passwordCorrect=false;
        } else if (password.length()>14){
            /*til_password.setError("");*/
            Toast.makeText(context, "密码长度超出14位", Toast.LENGTH_SHORT).show();
            passwordCorrect=false;
        } else if (!verifyPasswordByRegex(password)){
            /*til_password.setError("");*/
            Toast.makeText(context, "密码只能包含英语或数字", Toast.LENGTH_SHORT).show();
            passwordCorrect=false;
        }
